...
项目以"可运行、可演示、可公开交付"为目标，采用 `gateway + doc-service + index-service + ai-service + (optional python embedder / ocr)` 的结构。本地嵌入默认通过 JVM 内的 DJL + ONNX Runtime 直接运行，无需 Python；仅当 `KB_EMBEDDER_MODE=http`（Docker/K8s 部署默认值）时才需要 `kb-embedder` Python 服务。
...
- 索引服务负责知识库管理、BM25 + 向量混合检索，以及 embedding 调用（本地默认走 JVM 内 DJL，可切换到 Python `kb-embedder` HTTP）。
- AI 服务负责 RAG 编排、会话持久化和大模型调用。
- Python `kb-embedder` 提供可选的外置 embedding HTTP 接口（`KB_EMBEDDER_MODE=http` 时使用；Docker/K8s 默认如此）；`kb-ocr-service` 提供可选 OCR。
...
- `kb-index-service` 现在统一使用 pgvector 原生 `vector` 列存储 embedding，不再假设固定 `512` 维；这是因为云端托管嵌入模型可能返回 `4096` 维向量。
