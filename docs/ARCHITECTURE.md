...
系统由一个公网入口网关、三个核心 Java 微服务、一个可选 Python embedder、一个可选 OCR 服务，以及一个 PostgreSQL 数据库组成。本地嵌入默认由 `kb-index-service` 内置的 DJL + ONNX Runtime 直接完成，不依赖 Python；仅当 `KB_EMBEDDER_MODE=http`（Docker/K8s 默认）时才会启用独立的 `kb-embedder` 服务。可选开启 Eureka 注册中心实现服务发现。
...
Support services:
- kb-embedder:8090 (optional, only when KB_EMBEDDER_MODE=http)
...
- 计算 embedding：由 `LocalEmbeddingBackend` 策略接口实现（`kb.embedder.mode=djl` 走 JVM 内 DJL + ONNX Runtime，`http` 走 Python `kb-embedder`），或调用外部 OpenAI-compatible embeddings API
- 提供 BM25 + dense hybrid retrieval
- 默认融合权重：`bm25Weight=0.35`，`denseWeight=0.65`
- BM25 索引在内存中维护（按需从数据库重建）；数据库最终使用 pgvector 原生 `vector` 列存储 embedding，不再假设固定 `512` 维，以兼容 `4096` 维云端模型
...
### `kb-embedder` (`8090`, optional)

仅当 `KB_EMBEDDER_MODE=http` 时需要部署（Docker / K8s 默认如此；本地 IDEA 默认走 DJL 后端，可不启动此服务）。
...
4. `kb-index-service` 通过 `LocalEmbeddingBackend`（`djl` 走 JVM 内 DJL / `http` 走 `kb-embedder`）或外部 embeddings API 计算向量
...
- **Embedding 写入已改为严格失败**：知识库写入与重建索引阶段，如果 embedding 计算失败，请求会直接失败并回滚，不再像旧版本那样把空向量写入库中。
